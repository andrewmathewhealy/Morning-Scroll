...
const STYLES = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .observer-preview-card { border-radius: 24px; overflow: hidden; border: 1.5px solid #FDF2E8; }
  .observer-preview-ap { background: #FDF2E8; border-bottom: none; border-radius: 24px 24px 0 0; overflow: hidden; border: 1.5px solid #D62828; border-bottom: none; }
  .observer-preview-ap-headline { padding: 14px 16px 18px; }
  .observer-preview-ap-label { font-size: 13px; font-weight: 800; letter-spacing: 1.6px; text-transform: uppercase; color: #0C1A35; margin-bottom: 8px; }
  .observer-preview-ap-label span { text-decoration: underline #D62828 3px; text-underline-offset: 2px; letter-spacing: 0; margin-right: 1.6px; }
  .observer-preview-ap-title { font-family: 'Satoshi', sans-serif; font-size: 16px; color: #0C1A35; line-height: 1.4; font-weight: 600; }
  .observer-preview-bottom { background: linear-gradient(rgba(12,26,53,0.12), rgba(12,26,53,0.12)), #FDF2E8; padding: 20px; border-radius: 0 0 24px 24px; border: 1.5px solid #0C1A35; }
  .observer-preview-label { font-family: 'Instrument Serif', serif; font-size: 26px; font-weight: 400; letter-spacing: 0.5px; text-transform: lowercase; color: #0C1A35; margin-bottom: 10px; }
  .observer-preview-hl { font-family: 'Instrument Serif', serif; font-size: 18px; font-weight: 400; line-height: 1.3; color: #0C1A35; }
  .observer-preview-body { font-size: 14px; line-height: 1.6; color: #0C1A35; margin-top: 10px; }
...
// ── ALOOF OBSERVER EDITOR ─────────────────────────────────
function AloofObserverEditor() {
  const today = new Date().toISOString().slice(0, 10);
  const [date, setDate] = useState(today);
  ...
  const [observerHeadline, setObserverHeadline] = useState("");
  const [observerArticle, setObserverArticle] = useState("");
  const [saving, setSaving] = useState(false);
  const [saveMsg, setSaveMsg] = useState(null);
  ...
  const loadQueue = useCallback(async () => {
    setQueueLoading(true);
    try {
      const q = query(collection(db, "aloofObserver"), orderBy("date", "desc"));
      const snap = await getDocs(q);
      setQueue(snap.docs.map((d) => ({ id: d.id, ...d.data() })));
    } catch {}
    setQueueLoading(false);
  ...
  useEffect(() => {
    const existing = queue.find((q) => q.date === date);
    if (existing) {
      setApTitle(existing.apTitle || "");
      setApLink(existing.apLink || "");
      setObserverHeadline(existing.observerHeadline || "");
      setObserverArticle(existing.observerArticle || "");
  ...
  const handleSave = async () => {
    if (!date || !observerHeadline.trim() || !observerArticle.trim()) {
      setSaveMsg("Date, observer headline, and article are required.");
      return;
    }
    setSaving(true);
    setSaveMsg(null);
    try {
      ...
      if (imageFile) {
        const path = `observer/${date}-${Date.now()}.jpg`;
        const storageRef = ref(storage, path);
        await uploadBytes(storageRef, imageFile);
        imageUrl = await getDownloadURL(storageRef);
      }

      await setDoc(doc(db, "aloofObserver", date), {
        date,
        apTitle: apTitle.trim(),
        apLink: apLink.trim(),
        imageUrl,
        observerHeadline: observerHeadline.trim(),
        observerArticle: observerArticle.trim(),
        manual: true,
        createdAt: new Date().toISOString(),
      });
      setSaveMsg("Saved!");
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await deleteDoc(doc(db, "aloofObserver", id));
      loadQueue();
    } catch {}
  };
  ...
  return (
    <>
      {/* Date picker */}
      <div className="form-section">
        <div className="form-section-title">Dispatch Date</div>
        <input className="form-input" type="date" value={date} onChange={(e) => setDate(e.target.value)} />
      </div>

      {/* AP Headline */}
      <div className="form-section">
        <div className="form-section-title">AP Headline</div>
        <div>
          <label className="form-label">Headline</label>
          <input className="form-input" placeholder="AP News headline..." value={apTitle} onChange={(e) => setApTitle(e.target.value)} />
        </div>
        <div style={{ marginTop: "12px" }}>
          <label className="form-label">Link (optional)</label>
          <input className="form-input" placeholder="https://apnews.com/..." value={apLink} onChange={(e) => setApLink(e.target.value)} />
        </div>
      </div>

      {/* Image upload */}
      <div className="form-section">
        <div className="form-section-title">AP Image</div>
        <div
      ...
      {/* Observer content */}
      <div className="form-section">
        <div className="form-section-title">Aloof Observer Dispatch</div>
        <div>
          <label className="form-label">Headline</label>
          <input className="form-input" placeholder="Local Arrangement of Atoms Reports Feeling Unappreciated" value={observerHeadline} onChange={(e) => setObserverHeadline(e.target.value)} />
        </div>
        <div style={{ marginTop: "12px" }}>
          <label className="form-label">Article</label>
          <textarea
            className="form-input form-textarea"
            placeholder="3–4 sentences of the dispatch..."
            value={observerArticle}
            onChange={(e) => setObserverArticle(e.target.value)}
            rows={5}
      ...
      {/* Save */}
      <div className="save-row">
        <button className="save-btn" onClick={handleSave} disabled={saving}>{saving ? "Saving..." : "Save Dispatch"}</button>
        {saveMsg && <span className="save-status">{saveMsg}</span>}
      </div>
      ...
      <div className="preview-section">
        <div className="preview-label">Live Preview</div>
        <div className="preview-frame">
          <div className="observer-preview-card">
            <div className="observer-preview-ap">
              {imagePreview && (
                <div className="art-image" style={{ background: "#0a1a24" }}>
                  <img src={imagePreview} alt={apTitle} />
                </div>
              )}
              {apTitle && (
                <div className="observer-preview-ap-headline">
                  <div className="observer-preview-ap-label"><span>AP</span> News</div>
                  <div className="observer-preview-ap-title">{apTitle}</div>
                </div>
              )}
            </div>
            <div className="observer-preview-bottom">
              <div className="observer-preview-label">the aloof observer</div>
              <div className="observer-preview-hl">{observerHeadline || "Headline here..."}</div>
              {observerArticle && (
                <div className="observer-preview-body">{observerArticle}</div>
              )}
            </div>
          </div>
        </div>
      ...
      {/* Queue */}
      <div className="queue-section">
        <div className="queue-title">Dispatch Schedule</div>
        {queueLoading ? (
          <div className="queue-empty">Loading...</div>
        ) : queue.length === 0 ? (
          <div className="queue-empty">No dispatches yet</div>
        ) : (
          queue.map((item) => (
            <div className={`queue-item ${item.date === today ? "queue-today" : ""}`} key={item.id}>
              {item.imageUrl ? (
                <img src={item.imageUrl} alt={item.observerHeadline} className="queue-thumb" />
              ) : (
                <div className="queue-thumb" />
              )}
              <div className="queue-info">
                <div className="queue-date">{item.date}{item.date === today ? " — TODAY" : ""}</div>
                <div className="queue-art-title">{item.observerHeadline}</div>
                <div className="queue-art-meta">{item.apTitle}</div>
              </div>
              <button className="queue-delete" onClick={() => handleDelete(item.id)}>Delete</button>
            </div>
          ))
  ...
  return (
    <div className="admin-shell">
      ...
      <div className="admin-tabs">
        ...
        <button className={`admin-tab ${activeTab === "observer" ? "active" : ""}`} onClick={() => setActiveTab("observer")}>Aloof Observer</button>
      ...
      {activeTab === "observer" ? <AloofObserverEditor /> : activeTab === "journal" ? <JournalPromptEditor /> : activeTab === "polls" ? <PollEditor /> : <>
